({
  f9libLoaded: function (cmp, evt, helper) {
    ...
    function updateSalesforceIdCAV(salesforceId) {
      ...
      cons.log(`CAV update with cavList: ${JSON.stringify(updateCavList)}`);
    ...
    interactionApi.subscribe({
      ...
      callStarted: (startedCall) => {
        cons.log(`Call Started ${JSON.stringify(startedCall)}`);
        ...
        interactionApi
          ...
          .then((domainCavsReturned) => {
            ...
            cons.log(`Interaction API got cavList: ${JSON.stringify(cavDefinitions)}`);
          });
      },

      // on call accepted, get the domain CAVs and map to a dictionary for easy lookup
      callAccepted: (acceptedCall) => {
        cons.log(`Call Started ${JSON.stringify(acceptedCall)}`);

      },

      // on object selected, update the CAV
      objectSelected: (selectedObject) => {
        cons.log(`Object Selected ${JSON.stringify(selectedObject)}`);
        ...
        if (selectedObjectRecordId != undefined) {
          ...
          cons.debug(`URL Parts: ${JSON.stringify(urlParts)}`);
    ...
    hookApi.registerApi({
      ...
      afterScreenPop: function (screenPopData) {
        cons.log(`afterScreenPop ${JSON.stringify(screenPopData)}`);
      ...
      beforeDisposition: function (dispositionData) {
        cons.log(`beforeDisposition ${JSON.stringify(dispositionData)}`);
